...
def dist_matrix_by_shp(oShp, dShp, oEpsg, dEpsg, result, transMode=None):
    ...
    originsDf = originsDf if oEpsg == 4326 else proj(
        originsDf, None, 4326, api='pandas')
    
    destnatDf = destnatDf if dEpsg == 4326 else proj(
        destnatDf, None, 4326, api='pandas')
...
def pnt_to_facility(pnt, pntSrs, facilities, facSrs,
    ...
    pntDf = pntDf if pntSrs == 4326 else proj(pntDf, None, 4326, api='pandas')
    facil = facil if facSrs == 4326 else proj(facil, None, 4326, api='pandas')
    ...
    if pntSrs != 4326:
        pntDf = proj(pntDf, None, pntSrs, api='pandas')
    
    df_to_shp(pntDf, os.path.join(
        os.path.dirname(pnt), 
        f"{fprop(pnt, 'fn')}_result.shp"
    ))
