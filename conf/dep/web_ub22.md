Dependencies glass-desktop in Ubuntu 22.04
====================
...
**Change default port to 90 (HTTP) and 943 (HTTPS):**
Edit /etc/apache2/ports.conf and /etc/apache2/sites-available/000-default.conf
...
cd /opt && git clone https://github.com/GeoNode/geonode.git
cd /opt/geonode && git checkout 4.2.x
...
sudo apt install -y mapserver-bin cgi-mapserver


sudo mkdir /srv/geomoose
sudo chown pwgis:pwgis /srv/geomoose
cd /srv/geomoose
