...
# install PROJ - we need to compile because apt default version
# is not 9.4
sudo apt-get install libproj-dev proj-data proj-bin unzip -y
# 1) Instalar dependências básicas
sudo apt update
sudo apt install -y build-essential cmake git \
     libsqlite3-dev libcurl4-openssl-dev libtiff-dev

# 2) Clonar o repositório PROJ
cd /tmp
git clone https://github.com/OSGeo/PROJ.git
cd PROJ

# Opcional: mudar para versão estável desejada, por exemplo v9.4.0
git checkout 9.4.0
...
# 6) Verificar que instalou corretamente
proj -V
...
echo "export PROJ_DATA=/usr/local/share/proj" | sudo tee --append ~/.bashrc
...
sudo apt install postgresql-client-17 postgis postgresql-common postgresql-17 postgresql-17-postgis-3 netcat postgresql-17-ogr-fdw postgresql-17-postgis-3-scripts postgresql-plpython3-17 postgresql-17-pgrouting postgresql-server-dev-17 postgresql-17-cron -y
```
...
cd ~/ && tar -xzvf OTB-9.0.0-Linux.tar.gz --one-top-level=OTB_install
source ~/OTB_install/otbenv.profile
```
