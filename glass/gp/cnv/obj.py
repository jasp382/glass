...
def geodf_to_cnvhull(gdf, attr, geom):
    """
    Point GeoDataFrame to Convex Hull Geometry
    """

    from glass.prop.prj   import df_epsg
    from glass.dtt.mge.pd import merge_df

    epsg = df_epsg(gdf, geom)

    attr_val = gdf[attr].unique()

    ngdf = []
    for v in attr_val:
        _gdf = gdf[gdf[attr] == v]

        _gdf["geom_x"] = gdf[geom].x
        _gdf["geom_y"] = gdf[geom].y

        hull = ConvexHull(_gdf[["geom_x", "geom_y"]])

        poly = geometry.Polygon([[
            _gdf.geom_x.iloc[idx], _gdf.geom_y.iloc[idx]
        ] for idx in hull.vertices])

        cdf = gp.GeoDataFrame(
            [v], columns=[attr],
            crs=f"EPSG:{epsg}", geometry=[poly]
        )
