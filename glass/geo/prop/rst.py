...
def get_cellsize(rst, xy=False, bnd=None, gisApi='gdal'):
    ...
    * pygrass
    ...
    if gisApi == 'gdal':
        from osgeo           import gdal
        from glass.geo.prop.img import get_cell_size
...
def adjust_ext_to_snap(outExt, snapRst):
    ...
    from glass.geo.prop.df  import check_isShp, check_isRaster
    from glass.geo.prop.rst import rst_ext, get_cellsize
    from glass.geo.obj.g    import new_pnt, create_polygon
    ...
    if isRst:
        ...
        if isShp:
            from glass.geo.prop.feat import get_ext
...
def san_report_combine(report):
    ...
    from glass.dp.pd.fld import splitcol_to_newcols
