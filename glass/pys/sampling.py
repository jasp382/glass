...
def split_binparray_randomly(rnum, nd, rshp, proportion, min_sample=None, absvals=None):
    """
    ...
    if absvals:
    proportion = dict with number of pixels to extrac for each class
    """
    ...
    if not absvals:
        ...
        ncells_byval = [int(proportion[v]) for v in val if v in proportion]
    ...
    # Create result
    res  = np.full(rnum.shape, fill_value=nd, dtype=rnum.dtype)
    nres = np.full(rnum.shape, fill_value=nd, dtype=rnum.dtype)
