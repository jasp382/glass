...
def optimized_union_anls(lyr_a, lyr_b, outShp, ref_boundary,
    ...
    from glass.geo.df.sample     import create_fishnet
    ...
    if not multiProcess:
        ...
        # Export Data
        from glass.dct.geo.toshp.cff import grs_to_shp
...
def count_pntinpol(inpnt, inpoly, cntcol, out):
    ...
    # Export to file
    obj_to_shp(pol_df, "geometry", get_epsg_shp(inpoly), out)
...
def check_shape_diff(SHAPES_TO_COMPARE, OUT_FOLDER, REPORT, DB,
    ...
    * Dependencias:
    ...
    from glass.geo.df.tbl.fld   import rn_cols
...
def shp_diff_fm_ref(refshp, refcol, shps, out_folder,
    ...
    from glass.geo.wenv.grs     import run_grass
    from glass.pys.oss          import fprop
    from glass.geo.df.tbl.tomtx import tbl_to_areamtx
