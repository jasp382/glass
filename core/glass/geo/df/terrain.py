...
def make_dem(grass_workspace, data, field, output, extent_template,
    ...
    * CONTOUR;
    ...
    if method == "BSPLINE":
        from glass.geo.df.nop.itp import bspline

        # Convert to points if necessary
        if data_gtype != 'POINT' and data_gtype != 'MULTIPOINT':
            from glass.dct.geo.toshp.cgeo import feat_vertex_to_pnt
        ...
        from glass.geo.df.nop.itp import surfrst
        ...
        from glass.dct.geo.torst  import shp_to_rst
        from glass.geo.df.nop.itp import surfcontour
        
        # Apply mask if mask
        if mask:
            from glass.dct.geo.torst import grs_to_mask, rst_to_grs
        ...
        from glass.geo.df.nop.itp import ridw
