...
def rst_pnt_to_build(osmdb, pntTable, polyTable, api_db='SQLITE'):
    ...
    from glass.g.gp.ovl.sql import feat_within, feat_not_within
    ...
    if N11:
        ...
        rstBuild11 = shp_to_rst(
            grsBuild11, 11, "rst_builds11", cmd=True)
    ...
    for cls in lulcCls:
        ...
        rstb = shp_to_rst(
            shp, int(cls), f"rst_nbuild_{str(cls)}",
            cmd=True
        )
...
def num_assign_builds(osmdb, pntTbl, polTbl, folder, cells, srscode, rstT,
    ...
    if apidb == 'SQLITE':
        ...
        from glass.g.it.shp import dbtbl_to_shp as sel_by_attr
    from glass.g.gp.ovl.sql import feat_within, feat_not_within
    from glass.ng.sql.q     import q_to_obj
