...
def arcgis2geojson(arcgis, idAttribute=None):
    if isinstance(arcgis, str):
        return json.dumps(json_to_gjson(json.loads(arcgis), idAttribute))
    else:
        return json_to_gjson(arcgis, idAttribute)


def json_to_gjson(arcgis, idAttribute=None):
    """
    ...
    if 'features' in arcgis and arcgis['features']:
        ...
        for feature in arcgis['features']:
            geojson['features'].append(json_to_gjson(feature, idAttribute))
    ...
    if 'geometry' in arcgis or 'attributes' in arcgis:
        ...
        if 'geometry' in arcgis:
            geojson['geometry'] = json_to_gjson(arcgis['geometry'])
        else:
